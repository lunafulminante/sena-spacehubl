...
export default function DetalleEquipoPage() {
  ...
  return (
    <div className="max-w-lg mx-auto p-6 bg-ink-800/70 border border-violet-400/15 rounded-2xl backdrop-blur-sm text-white shadow-xl">
      <h2 className="text-xl font-bold text-violet-300 mb-1">Editar Equipo</h2>
      <p className="text-xs text-ink-400 mb-4 font-mono">Placa SENA: <span className="text-white font-bold">{placaSena}</span></p>
      ...
      <form onSubmit={handleUpdate} className="space-y-4 font-mono text-xs">
        ...
        <div>
          <label className="block font-bold text-ink-300 mb-1">Estado Técnico</label>
        ...
        <div>
          ...
          <input type="text" value={ambiente} onChange={(e) => setAmbiente(e.target.value)} className="w-full bg-ink-900 border border-ink-700 rounded-xl p-2.5 text-white focus:outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-500/30" />
        </div>
        <div className="pt-2 flex justify-end gap-3 font-sans">
          <button type="button" onClick={() => navigate('/inventario')} className="px-4 py-2 bg-ink-700 hover:bg-ink-600 rounded-xl text-xs font-bold text-ink-300">Volver</button>
          <button type="submit" className="px-4 py-2 bg-linear-to-r from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 text-white font-bold rounded-xl text-xs shadow-lg shadow-violet-900/40">Guardar Cambios</button>
        </div>
      </form>
    </div>
  );
